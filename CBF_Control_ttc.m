function [u] = CBF_Control_ttc(X,u_ref,alpha1,alpha2,alpha3,s_star,v_star,tilde_vh)%,v_star,tilde_vh
%time-to-collision
A=[];
b=[];
state_size=size(X);
...
for index=1:state_size(1)/2
    ...
    if index~=1
        ...
        s_safe=tau*(X(index*2)-X(index*2-2));
    else
        s_safe=tau*(X(2)-tilde_vh);
    end
    delta_b(index*2-1)=1;
    delta_b(index*2)=-tau;
%     if index==3
